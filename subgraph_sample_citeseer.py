...
def load_graph(dataset_str = 'citeseer'):
...
def subgraph_sample(init_idx_list=None, number_subgraph=50, kstep=7, ratio=0.1, pn=0.5):
        ...
        while(itera < number_subgraph):
            ...
            if(len(index_subgraph)>1000):
                ...
                subgraph_set_dict[itera] = {'index_subgraph': torch.LongTensor(index_subgraph), 'adj':adj, 'idx_train': torch.LongTensor(idx_train_subgraph), 'idx_val': torch.LongTensor(idx_val_subgraph),
                        'idx_test': torch.LongTensor(idx_test_subgraph), 
                        'idx_train_list': idx_train_list, 'idx_test_list': idx_test_list}
                itera += 1
...
if __name__ == "__main__":
    ...
    sampled_subgraph_dict,subgraph_size = subgraph_sample(init_idx_list=None,number_subgraph=50,kstep=4,ratio=0.2)
    print('avg subgraph size', sum(subgraph_size)/len(subgraph_size))
    print('num graph', len(subgraph_size))
    f = open('./sampled_subgraph/citeseer_full', 'wb')
