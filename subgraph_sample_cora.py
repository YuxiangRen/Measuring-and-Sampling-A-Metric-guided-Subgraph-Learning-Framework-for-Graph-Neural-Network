...
def load_graph(dataset_str = 'cora'):
...
def subgraph_sample(init_idx_list=None, number_subgraph=50, kstep=4, ratio=0.2, pn=0.5):
    ...
    if(init_idx_list is None):
        ...
        while(itera < number_subgraph):
            #select initial node
            if(len(index_subgraph)<201):
                index_subgraph = list(set(index_subgraph + [np.random.randint(640, 1640)]))
            else:
                index_subgraph = [np.random.randint(640, 1640)]
            for step in range(kstep):
                ...
                for i in index_subgraph:
                    ...
                    for potential in graph[i]:
                        if((potential not in index_subgraph) & to_add(features, poten_list, potential, smoothness, ratio=ratio)):
            ...
            if(len(index_subgraph)>200):
                ...
                subgraph_set_dict[itera] = {'index_subgraph': torch.LongTensor(index_subgraph), 'adj':adj, 'idx_train': torch.LongTensor(idx_train_subgraph), 'idx_val': torch.LongTensor(idx_val_subgraph),
                        'idx_test': torch.LongTensor(idx_test_subgraph), 'idx_train_list':idx_train_list,
        ...
        for init_idx in init_idx_list:
            ...
            for step in range(kstep):
                ...
                index_subgraph = index_subgraph + neighbors
                index_subgraph = list(set(index_subgraph))
...
if __name__ == "__main__":
    ...
    sampled_subgraph_dict,subgraph_size = subgraph_sample(init_idx_list=None,number_subgraph=100,kstep=7,ratio=0)
    print('avg subgraph size', sum(subgraph_size)/len(subgraph_size))
    print('num graph', len(subgraph_size))
    f = open('./sampled_subgraph/cora_metricguide_kstep9_sr01_pn08_num100_0', 'wb')
