...
# Load data
AAAA, features, labels, idx_train, idx_val, idx_test = load_data('pubmed')
...
from subgraph_sample_pubmed import subgraph_sample
# destination_folder_path = './sampled_subgraph/'
# destination_file_name_g = 'pubmed_metricguide_kstep7_sr0_pn08_num100_0'
...
# g.close()
content_g, subgraph_size_load = subgraph_sample(number_subgraph=50, kstep=3, ratio=0.2, pn=0.6)
f_tosave = open('./results_pubmed/pubmed_mg-gcn_kstep3_sr04_pn05_num50_0', 'wb')
...
for epoch in range(args.epochs):
    for subgraph in content_g:
        ...
        torch.save(model.state_dict(), '{}.pubmedpkl'.format(iter))
...
model.load_state_dict(torch.load('{}.pubmedpkl'.format(best_epoch)))
files = glob.glob('*.pubmedpkl')
for file in files:
    os.remove(file)
...
if(args.test_format == "vote"):
    ...
    # resample subgraphs for the test sample without corresponding subgraphs
    if(len(no_test_list) > 0):
        subgraph_set_dict, subgraph_size = subgraph_sample(init_idx_list = no_test_list ,kstep=3, ratio=0, pn=0.8)
        with torch.no_grad():
            model.eval()
            for i in no_test_list:
                index = subgraph_set_dict[i]['index_subgraph']
    ...
    print("Fianl Concat format Test set results:", "acc= {:.4f}".format(acc_test))

with torch.no_grad():
        model.eval()
        output_fulltest = model(features, AAAA)
        acc_test_fullgraph = accuracy(output_fulltest[idx_test], labels[idx_test])
